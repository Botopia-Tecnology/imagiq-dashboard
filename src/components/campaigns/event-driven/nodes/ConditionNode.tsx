...
const ConditionNode = memo(({ data, selected }: NodeProps<ConditionNodeData>) => {
  ...
  const [config, setConfig] = useState<ConditionNodeData['config']>(data.config || {
    conditions: [],
    operator: 'AND'
  });
  ...
  const generateSummary = (conditions: FilterCondition[], operator: 'AND' | 'OR'): string => {
        ...
        {config.conditions && config.conditions.length > 0 ? (
          <div className="p-3 w-full flex flex-col items-center justify-center gap-1.5 overflow-hidden">
            ...
            <span className="text-xs font-medium text-center">
              {config.conditions.length} Condición{config.conditions.length !== 1 ? 'es' : ''}
            </span>
            ...
            <div className="flex flex-col items-center gap-0.5 w-full max-h-[60px] overflow-y-auto">
              {config.conditions.map((cond, idx) => {
                ...
                return (
                  <span key={idx} className="text-[10px] text-center text-muted-foreground px-1 leading-tight truncate max-w-full">
